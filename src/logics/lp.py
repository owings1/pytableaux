...
class TableauxRules(object):
    ...
    class GapClosure(logic.TableauxSystem.ClosureRule):
        """
    ...
    class ConjunctionNegatedDesignated(fde.TableauxRules.ConjunctionNegatedDesignated):
        ...
        def score_candidate_map(self, target):
            ...
            if not self.designation:
                ...
                scores.update({
                    'b1': scores['b1'] or branch.has({'sentence': s.lhs, 'designated': False}),
                    'b2': scores['b2'] or branch.has({'sentence': s.rhs, 'designated': False}),
                })
            return scores
    ...
    class ConjunctionUndesignated(fde.TableauxRules.ConjunctionUndesignated):
        ...
        def score_candidate_map(self, target):
            ...
            if not self.designation:
                ...
                s = self.sentence(target['node'])
                scores.update({
                    'b1': scores['b1'] or branch.has({'sentence': negative(s.lhs), 'designated': False}),
                    'b2': scores['b2'] or branch.has({'sentence': negative(s.rhs), 'designated': False}),
                })
            return scores
    ...
    class MaterialConditionalNegatedUndesignated(fde.TableauxRules.MaterialConditionalNegatedUndesignated):
        ...
        def score_candidate_map(self, target):
            scores = super(TableauxRules.MaterialConditionalNegatedUndesignated, self).score_candidate_map(target)
            if not self.designation:
                # only apply to implementations for undesignated rules
                branch = target['branch']
                s = self.sentence(target['node'])
