...
class TestModelRefactorBugs(Base):
    ...
    def test_model_branch_proof_deny_antec(self):
        ...
        m = self.m(tab.open[0])
        s = Atomic(0, 0)
        self.assertEqual(m[s, 0], 'F')
        self.assertEqual(m[~s, 0], 'T')
...
class TestModelAtomics(Base):
    ...
    def test_model_branch_no_proof_atomic(self):
        ...
        b += swnode(s, 0)
        m.read_branch(b)
        self.assertEqual(m[Atomic(0, 0), 0], 'T')
...
class TestModelModalAccess(Base):
    ...
    def test_model_branch_no_proof_access(self):
        ...
        b += anode(0, 1)
        m = self.m(b)
        self.assertTrue(m.R.has((0,1)))

    def test_model_access_sees(self):
        m = self.m()
        m.R.add((0,0))
